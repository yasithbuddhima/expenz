...
class IncomeServices {
  ...
  static Future<void> saveIncome(Income income, BuildContext context) async {
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Error Adding the Income"),
            duration: Duration(seconds: 2),
          ),
        );
      }
  ...
  static Future<void> deleteIncome(int id, BuildContext context) async {
    try {
      SharedPreferences prefs = await SharedPreferences.getInstance();
      List<String>? existingIncomes = prefs.getStringList(_incomeKey);
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Error deleting the Income"),
            duration: Duration(seconds: 2),
          ),
        );
      }
  ...
  static Future<void> deleteAllIncomes(BuildContext context) async {
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Error deleting all the Income"),
            duration: Duration(seconds: 2),
          ),
        );
      }
